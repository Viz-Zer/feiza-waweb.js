...
module.exports = {
    ...
    start: async(feiza, m, { mime, quoted }) => {
        if (!/webp/.test(mime)) return m.reply(`Support mime webp only`)
        ...
        if (!quoted._data.isAnimated) {
            ...
            exec(`ffmpeg -i ${dir} ${ran}`, (err) => {
                ...
                feiza.sendMessage(m.from, media, { caption: `Sticker to Image`, quotedMessageId: m.id._serialized })
                fs.unlinkSync(ran)
            })
            ...
            feiza.sendMessage(m.from, media, { caption: `Sticker to Video`, quotedMessageId: m.id._serialized })
            fs.unlinkSync(dir)
        }
    }
}
