...
module.exports = async (feiza, m, commands) => {
    try {
        ...
        const isOwner = [feiza.info.wid._serialized, ...global.owner].map(v => v.replace(/[^0-9]/g, '') + '@c.us').includes(sender)
        ...
        const isBotAdmin = isGroup ? groupAdmins.includes(feiza.info.wid._serialized) : false
        const isAdmin = isGroup ? groupAdmins.includes(sender) : false

        if (m) {
            console.log(chalk.black(chalk.bgWhite('[ MESSAGE ]')), chalk.black(chalk.bgGreen(new Date(m._data.t * 1000))), chalk.black(chalk.bgGreen(body || type)) + "\n" + chalk.black(chalk.bgWhite("=> from")), chalk.black(chalk.bgGreen(m._data.notifyName)), chalk.black(chalk.yellow(sender)) + "\n" + chalk.black(chalk.bgWhite("=> In")), chalk.bgGreen(isGroup ? groupName : m._data.notifyName, from))  
        }

        //if (options.autoRead) (feiza.type == "legacy") ? await feiza.chatRead(m.key, 1) : await feiza.sendReadReceipt(from, sender, [m.id])
